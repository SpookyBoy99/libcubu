  ...
  , texDescr_{}
{
  ...
  gpuAssert(cudaMalloc((void**)&devData_, size * sizeof(T)));
  ...
  gpuAssert(cudaCreateTextureObject(&tex_, &texRes_, &texDescr_, nullptr));
}
...
linear_resource<T>::~linear_resource()
{
  ...
  gpuAssert(cudaDestroyTextureObject(tex_));
  gpuAssert(cudaFree(devData_));
}

template<class T>
void
linear_resource<T>::copy_to_device(const std::vector<T>& hostData)
{
...
template<class T>
void
linear_resource<T>::copy_to_device(const T* hostData)
{
  gpuAssert(
    cudaMemcpy(devData_, hostData, size_ * sizeof(T), cudaMemcpyHostToDevice));
}

template<class T>
void
linear_resource<T>::copy_to_host(std::vector<T>& destData)
{
...
template<class T>
void
linear_resource<T>::copy_to_host(T* destData)
{
  gpuAssert(
    cudaMemcpy(destData, devData_, size_ * sizeof(T), cudaMemcpyDeviceToHost));
}
